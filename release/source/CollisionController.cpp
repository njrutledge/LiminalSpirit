        ...
        if (*(attack->getSensorName()) == "playerattacksensor") {
            if (Mirror* mirror = dynamic_cast<Mirror*>(enemy)) {
                ...
                if (attack->getType() == AttackController::p_exp_package) {
                    AC->createAttack(attack->getPosition() /*cugl::Vec2(bd->getPosition().x, bd->getPosition().y)*/, 3, 0.1, 40, AttackController::p_exp, cugl::Vec2::ZERO, timer, PLAYER_RANGE, PLAYER_RANGE_FRAMES);
                }
