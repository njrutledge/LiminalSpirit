...
bool AttackController::Attack::init(const cugl::Vec2 p, float radius, float a, float dmg, float scale, Type s, MeleeState m, cugl::Vec2 oof, cugl::PolyFactory b, cugl::Vec2 vel, float angle, float timer, string attackID) {
    ...
    if (_rangedCounter > _reload) {
        switch (attack) {
            case SwipeController::leftAttack:
                _pending.emplace(Attack::alloc(p, 0.6, 0.5, 2, _scale, Type::p_range, first, _leftOff, ballMakyr, cugl::Vec2(_p_vel).rotate(angleAdjusted * M_PI / 180), angle, left, timer, PLAYER_RANGE));
                _rangedCounter = 0;
                sound->play_player_sound(SoundController::playerSType::shoot);
                break;
            case SwipeController::rightAttack:
                _pending.emplace(Attack::alloc(p, 0.6, 0.5, 2, _scale, Type::p_range, first, _rightOff, ballMakyr, cugl::Vec2(_p_vel).rotate(angleAdjusted * M_PI / 180), angle, right, timer, PLAYER_RANGE));
                _rangedCounter = 0;
                sound->play_player_sound(SoundController::playerSType::shoot);
                break;
            case SwipeController::upAttack:
                _pending.emplace(Attack::alloc(p, 0.6, 0.5, 2, _scale, Type::p_range, first, _upOff, ballMakyr, cugl::Vec2(_p_vel).rotate(angleAdjusted * M_PI / 180), angle, up, timer, PLAYER_RANGE));
                _rangedCounter = 0;
                sound->play_player_sound(SoundController::playerSType::shoot);
                break;
            case SwipeController::downAttack:
                if(!grounded){
                _pending.emplace(Attack::alloc(p, 0.6, 0.5, 2, _scale, Type::p_range, first, _downOff, ballMakyr, cugl::Vec2(_p_vel).rotate(angleAdjusted * M_PI / 180), angle, down, timer, PLAYER_RANGE));
                } else{
                    _pending.emplace(Attack::alloc(p, 0.6, 0.1, 2, _scale, Type::p_range, first, _leftOff, ballMakyr, cugl::Vec2(_p_vel).rotate(M_PI * 0.5), 180, left, timer, PLAYER_RANGE));
                    _pending.emplace(Attack::alloc(p, 0.6, 0.1, 2, _scale, Type::p_range, first, _rightOff, ballMakyr, cugl::Vec2(_p_vel).rotate(M_PI * 1.5), 0, right, timer, PLAYER_RANGE));
                }
                _rangedCounter = 0;
                sound->play_player_sound(SoundController::playerSType::shoot);
                break;
            case SwipeController::chargedLeft:
                _pending.emplace(Attack::alloc(p, 0.3, 1.5, 0, _scale, Type::p_exp_package, first, _leftOff, ballMakyr, cugl::Vec2(_c_vel).rotate(angleAdjusted * M_PI / 180), angle, left, timer, PLAYER_RANGE));
                _rangedCounter = 0;
                break;
            case SwipeController::chargedRight:
                _pending.emplace(Attack::alloc(p, 0.3, 1.5, 0, _scale, Type::p_exp_package, first, _rightOff, ballMakyr, cugl::Vec2(_c_vel).rotate(angleAdjusted * M_PI / 180), angle, right, timer, PLAYER_RANGE));
                _rangedCounter = 0;
                break;
            case SwipeController::chargedUp:
                _pending.emplace(Attack::alloc(p, 0.3, 1.5, 0, _scale, Type::p_exp_package, first, _upOff, ballMakyr, cugl::Vec2(_c_vel).rotate(angleAdjusted * M_PI / 180), angle, up, timer, PLAYER_RANGE));
                _rangedCounter = 0;
                break;
            case SwipeController::chargedDown:
                _pending.emplace(Attack::alloc(p, 2, 0.2, 4, _scale, Type::p_range, first, _leftOff,  ballMakyr, cugl::Vec2::ZERO, 180, left, timer, PLAYER_RANGE));
                _pending.emplace(Attack::alloc(p, 2, 0.2, 4, _scale, Type::p_range, first, _rightOff, ballMakyr, cugl::Vec2::ZERO, 0, right, timer, PLAYER_RANGE));
                _rangedCounter = 0;
                break;
            default:
                break;
        ...
        switch (attack) {
            case SwipeController::leftAttack:
                if (_melee == cool) {
                    ...
                    _pending.emplace(Attack::alloc(p, 1.5, 0.1, 1, _scale, Type::p_melee, h2_left, _leftOff, ballMakyr, cugl::Vec2::ZERO, 180, left, timer, PLAYER_MELEE));
                    _meleeCounter = 0;
                    _melee = h3_left;
                    sound->play_player_sound(SoundController::playerSType::slash);
                } else if (_melee == h3_left && _multiCounter < _hit_window) {
                    _pending.emplace(Attack::alloc(p, 2, 0.1, 3, _scale, Type::p_melee, h3_left, _leftOff, ballMakyr, cugl::Vec2::ZERO, 180, left, timer, PLAYER_MELEE));
                    _meleeCounter = 0;
                    _melee = cool;
                    sound->play_player_sound(SoundController::playerSType::slash);
                } else {
                    _pending.emplace(Attack::alloc(p, 1, 0.1, 1, _scale, Type::p_melee, h1_left, _leftOff, ballMakyr, cugl::Vec2::ZERO, 180, left, timer, PLAYER_MELEE));
                    _meleeCounter = 0;
                    _melee = h2_left;
                    sound->play_player_sound(SoundController::playerSType::slash);
                }
            ...
            case SwipeController::rightAttack:
                if (_melee == cool) {
                    ...
                    _pending.emplace(Attack::alloc(p, 1.5, 0.1, 1, _scale, Type::p_melee, h2_right, _rightOff, ballMakyr, cugl::Vec2::ZERO, 0, right, timer, PLAYER_MELEE));
                    _meleeCounter = 0;
                    _melee = h3_right;
                    sound->play_player_sound(SoundController::playerSType::slash);
                } else if (_melee == h3_right && _multiCounter < _hit_window) {
                    _pending.emplace(Attack::alloc(p, 2, 0.1, 3, _scale, Type::p_melee, h3_right, _rightOff, ballMakyr, cugl::Vec2::ZERO, 0, right, timer, PLAYER_MELEE));
                    _meleeCounter = 0;
                    _melee = cool;
                    sound->play_player_sound(SoundController::playerSType::slash);
                } else {
                    _pending.emplace(Attack::alloc(p, 1, 0.1, 1, _scale, Type::p_melee, h1_right, _rightOff, ballMakyr, cugl::Vec2::ZERO, 0, right, timer, PLAYER_MELEE));
                    _meleeCounter = 0;
                    _melee = h2_right;
                    sound->play_player_sound(SoundController::playerSType::slash);
                }
...
//                }
                break;
            case SwipeController::chargedLeft:
                _pending.emplace(Attack::alloc(p, 1.25, 0.5, 4, _scale, Type::p_dash, first, _leftOff + Vec2(-0.5,0), ballMakyr, cugl::Vec2(-20,0), 180, left, timer, PLAYER_MELEE));
                _meleeCounter = 0;
                _melee = cool;
                break;
            case SwipeController::chargedRight:
                _pending.emplace(Attack::alloc(p, 1.25, 0.5, 4, _scale, Type::p_dash, first, _rightOff + Vec2(0.5,0), ballMakyr, cugl::Vec2(20,0), 0, right, timer, PLAYER_MELEE));
                _meleeCounter = 0;
                _melee = cool;
                break;
            case SwipeController::chargedUp:
                _pending.emplace(Attack::alloc(p, 1.25, 0.5, 4, _scale, Type::p_dash, first, _upOff + Vec2(0,0.5), ballMakyr, cugl::Vec2(0,20), 90, up, timer, PLAYER_MELEE));
                _meleeCounter = 0;
                _melee = cool;
                break;
            case SwipeController::chargedDown:
                if(!grounded){
                _pending.emplace(Attack::alloc(p, 1.25, 0.5, 4, _scale, Type::p_dash, first, _downOff + Vec2(0,-0.5), ballMakyr, cugl::Vec2(0,-20), 270, down, timer, PLAYER_MELEE));
                    _meleeCounter = 0;
                } else {
                    _pending.emplace(Attack::alloc(p, 2, 0.05, 2, _scale, Type::p_melee, first, _leftOff, ballMakyr, cugl::Vec2::ZERO, 180, left, timer, PLAYER_MELEE));
                    _meleeCounter = 0;
                    _pending.emplace(Attack::alloc(p, 2, 0.05, 2, _scale,  Type::p_melee, first, _rightOff, ballMakyr, cugl::Vec2::ZERO, 0, right, timer, PLAYER_MELEE));
                    _meleeCounter = 0;
                }
                _melee = cool;
                break;
...
void AttackController::createAttack(cugl::Vec2 p, float radius, float age, float damage, Type t, cugl::Vec2 vel, float timer, string attackID) {
    std::shared_ptr<Attack> attack = Attack::alloc(p, radius, age, damage, _scale, t, first, cugl::Vec2::ZERO, ballMakyr, vel, 0, neither, timer, attackID);
    attack->setSplitable(true);
    _pending.emplace(attack);
}

void AttackController::createAttack(cugl::Vec2 p, float radius, float age, float damage, Type t, cugl::Vec2 vel, float timer, string attackID, bool splitable) {
    std::shared_ptr<Attack> attack = Attack::alloc(p, radius, age, damage, _scale, t, first, cugl::Vec2::ZERO, ballMakyr, vel, 0, neither, timer, attackID);
