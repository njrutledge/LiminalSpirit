    ...
    if (_rangedCounter > _reload) {
        switch (attack) {
            case SwipeController::leftAttack:
                _pending.emplace(Attack::alloc(p, 0.6, 0.5, 10, _rscale, Type::p_range, first, _leftOff, ballMakyr, cugl::Vec2(_p_vel).rotate(angleAdjusted * M_PI / 180), angle, left, timer, PLAYER_RANGE, PLAYER_RANGE_FRAMES));
                _rangedCounter = 0;
                sound->play_player_sound(SoundController::playerSType::shoot);
                break;
            case SwipeController::rightAttack:
                _pending.emplace(Attack::alloc(p, 0.6, 0.5, 10, _rscale, Type::p_range, first, _rightOff, ballMakyr, cugl::Vec2(_p_vel).rotate(angleAdjusted * M_PI / 180), angle, right, timer, PLAYER_RANGE, PLAYER_RANGE_FRAMES));
                _rangedCounter = 0;
                sound->play_player_sound(SoundController::playerSType::shoot);
                break;
            case SwipeController::upAttack:
                _pending.emplace(Attack::alloc(p, 0.6, 0.5, 10, _rscale, Type::p_range, first, _upOff, ballMakyr, cugl::Vec2(_p_vel).rotate(angleAdjusted * M_PI / 180), angle, up, timer, PLAYER_RANGE, PLAYER_RANGE_FRAMES));
                _rangedCounter = 0;
                sound->play_player_sound(SoundController::playerSType::shoot);
                break;
            case SwipeController::downAttack:
                if(!grounded){
                    _pending.emplace(Attack::alloc(p, 0.6, 0.5, 10, _rscale, Type::p_range, first, _downOff, ballMakyr, cugl::Vec2(_p_vel).rotate(angleAdjusted * M_PI / 180), angle, down, timer, PLAYER_RANGE, PLAYER_RANGE_FRAMES));
                } else{
