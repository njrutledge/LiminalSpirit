...
void ParticlePool::newParticle() {
	...
	if (_numTex > 0) {
		_particles.push_back(Particle::alloc(_emissionPoint, _gravity, speed, lifetime, angle, _numTex));
	}
	else {
		_particles.push_back(Particle::alloc(_emissionPoint, _gravity, speed, lifetime, angle));
	}
}
